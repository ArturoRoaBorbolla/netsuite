...
class SuiteSql:      
    ...
    def generate_statement(self,sql):
        return f'{{"q" : "{sql}" }}'
    ...
    def execute(self,command):
        self.init_signature(NS_ACCOUNT=NS_ACCOUNT,NS_CONSUMER_KEY=NS_CONSUMER_KEY,NS_CONSUMER_SECRET=NS_CONSUMER_SECRET,NS_TOKEN_KEY=NS_TOKEN_KEY,NS_TOKEN_SECRET=NS_TOKEN_SECRET,oauth_signature_method=oauth_signature_method,oauth_version = oauth_version,oauth_timestamp = oauth_timestamp ,oauth_nonce = oauth_nonce)
        self.create_parameter_string()
        self.encoded_parameter_string = urllib.parse.quote(self.parameter_string, safe='')
        url = 'https://4455308-sb1.suitetalk.api.netsuite.com/services/rest/query/v1/suiteql'
        ...
        response = requests.post(url, data=self.generate_statement(command),headers=headers)
...
class SuiteSearch:
    ...
    <soapenv:Body>
        <search xsi:type='platformMsgs:SearchRequest'>
            <searchRecord xsi:type='listEmp:EmployeeSearchAdvanced' savedSearchId='{searchid}'/>
        </search>
    </soapenv:Body>
    ...
    def execute(self):
        ...
        soap=self.get_soap(encoded_oauth_signature,96,oauth_nonce,str(int(time.time())))
        print(soap)
        time.sleep(3)
